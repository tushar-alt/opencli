...
const BASE_SYSTEM_PROMPT = `You are Species, an expert AI coding assistant running in the terminal.
...
function walkDir(
  ...
  let entries: string[];
  try {
    entries = readdirSync(dir).filter((e) => !e.startsWith('.') || e === '.species.json');
  } catch {
    return;
  }
